...
const ART_ADJ: &[&str] = &["Sunless", "Weeping", "Ninefold", "Iron", "Ashen", "Starlit", "Serpent", "Hollow", "Bright", "Black", "Unsleeping", "Glass", "Thunder", "Moon", "Ember", "Silent"];
    ...
    pub fn artifacts_of(&self, p: usize) -> Vec<usize> {
        self.artifacts
            ...
            .filter(|a| match a.holder {
                Holder::Polity(x) => x == p,
                Holder::Person(per) => self.persons[per].alive() && self.persons[per].polity == Some(p) && self.polities[p].ruler == Some(per),
...
pub fn make_artifact(w: &mut World, kind: ArtifactKind, maker: Option<usize>, polity: Option<usize>, holder: Holder, occasion: &str) -> usize {
    let rng = w.rng.clone();
    let id = w.artifacts.len();
    let lang = polity.map(|p| w.polity_lang(p).clone()).or_else(|| maker.map(|m| w.cultures[w.persons[m].culture].lang.clone())).unwrap_or_else(|| w.cultures[0].lang.clone());
    ...
    let name = match rng.below(4) {
        ...
        _ => format!("the {} {} of {}", adj, kind.word(), polity.and_then(|p| w.polities[p].capital).map(|c| w.cities[c].name.clone()).unwrap_or_else(|| lang.name(&rng))),
    };
    let description = match kind {
        ...
        ArtifactKind::Banner => "a standard that has never been taken in battle, whatever the outcome",
    ...
    .to_string();
    let lost_at = if let Holder::Lost = holder { polity.and_then(|p| w.capital_cell(p)) } else { None };
...
pub fn artifacts_on_capture(w: &mut World, city: usize, winner: usize, loser: usize) -> String {
    let was_capital = w.polities[loser].capital == Some(city) || w.polities[loser].capital.is_none();
    ...
    for a in taken {
        ...
        if w.rng.chance(0.25) {
            ...
            artifact_passes(w, a, Holder::Lost, &format!("vanished in the sack of {}.", w.cities[city].name));
            s.push_str(&format!(" {} vanished in the confusion.", crate::lang::capitalize(&name)));
        } else {
            artifact_passes(w, a, Holder::Polity(winner), &format!("was carried off to {} as spoils.", w.polities[winner].short));
            s.push_str(&format!(" {} was carried off by the victors.", crate::lang::capitalize(&name)));
        }
    }
    s
}
...
pub fn artifacts_on_fall(w: &mut World, p: usize, absorbed_by: Option<usize>) {
    for a in w.artifacts_of(p) {
        match absorbed_by {
            Some(q) if w.rng.chance(0.7) => artifact_passes(w, a, Holder::Polity(q), &format!("passed to {} with the fall of {}.", w.polities[q].short, w.polities[p].short)),
            _ => {
                w.artifacts[a].lost_at = w.capital_cell(p);
                artifact_passes(w, a, Holder::Lost, &format!("was lost in the ruin of {}.", w.polities[p].name));
            }
        }
    }
}
...
pub fn artifacts_on_ruler_death(w: &mut World, p: usize, r: usize) {
    ...
    let great = matches!(epithet.as_str(), "the Great" | "the Conqueror" | "the Wise" | "the Lawgiver" | "the Builder" | "the Victorious" | "the Lion");
    if great && rng.chance(0.25) && w.artifacts_of(p).len() < 2 && w.artifacts.len() < 3 + w.living_polities().len() / 2 {
        let kind = *rng.pick(&[ArtifactKind::Crown, ArtifactKind::Blade, ArtifactKind::Banner, ArtifactKind::Horn]);
        let name = w.persons[r].full_name();
        let capital = w.polities[p].capital.map(|c| w.cities[c].name.clone()).unwrap_or_else(|| w.polities[p].short.clone());
        make_artifact(w, kind, Some(r), Some(p), Holder::Polity(p), &format!("In memory of {}, the smiths of {} laboured a year and", name, capital));
    }
}

pub fn artifacts_on_wonder(w: &mut World, p: usize, cap: usize) {
    if w.rng.chance(0.3) && w.artifacts_of(p).len() < 2 && w.artifacts.len() < 3 + w.living_polities().len() / 2 {
        let kind = *w.rng.pick(&[ArtifactKind::Gem, ArtifactKind::Chalice, ArtifactKind::Mirror, ArtifactKind::Crown]);
        let city = w.cities[cap].name.clone();
        make_artifact(w, kind, w.polities[p].ruler, Some(p), Holder::City(cap), &format!("To crown the new wonder of {},", city));
    }
}
...
pub fn artifacts_on_school(w: &mut World, s: usize) {
    ...
    if w.rng.chance(0.25) {
        let kind = if w.schools[s].kind == super::SchoolKind::Arcane { *w.rng.pick(&[ArtifactKind::Staff, ArtifactKind::Tome, ArtifactKind::Mirror]) } else { *w.rng.pick(&[ArtifactKind::Tome, ArtifactKind::Chalice, ArtifactKind::Horn]) };
        let sname = w.schools[s].short.clone();
        make_artifact(w, kind, Some(founder), polity, Holder::City(city), &format!("For the {} of {},", w.schools[s].kind.follower(), sname));
    }
}

pub fn artifacts_on_catastrophe(w: &mut World, cell: usize, s: usize) {
    let founder = w.schools[s].founder;
    let id = make_artifact(w, ArtifactKind::Shard, Some(founder), None, Holder::Lost, "Among the glass of the unmade city,");
    ...
    for a in 0..w.artifacts.len() {
        match w.artifacts[a].holder {
            Holder::Lost => {
                ...
                let finder = w.cells[at].owner.filter(|&p| w.polities[p].alive()).or_else(|| {
                    let mut best = None;
                    let mut best_d = 30;
                ...
                if let Some(p) = finder {
                    let finder_desc = *rng.pick(&["shepherds", "grave-robbers", "a ploughman", "children", "soldiers digging a well", "a hermit"]);
                    let place = w.place_phrase(at, Some(p));
                    let how = format!("was found: {} of {} dug it out of the earth {}.", finder_desc, w.polities[p].short, place);
            ...
            Holder::Polity(_) | Holder::City(_) if rng.chance(0.0015) => {
                ...
                let how = match rng.below(3) {
                    0 => format!("was stolen from the vaults of {} by a thief who was never caught.", w.polities[p].short),
                    1 => format!("was lost when the ship carrying it from {} foundered.", w.polities[p].short),
                    _ => format!("vanished from {}; the guards swore the doors had never opened.", w.polities[p].short),
                };
            ...
            Holder::Person(per) if !w.persons[per].alive() => {
                ...
                artifact_passes(w, a, Holder::Lost, &format!("was buried with {} and its resting place forgotten.", w.persons[per].full_name()));
            }
            _ => {}
        }
    }
    ...
    for p in w.living_polities() {
        ...
        for &a in &held {
            ...
            match w.artifacts[a].kind {
                ArtifactKind::Crown | ArtifactKind::Banner | ArtifactKind::Horn => stab += 0.01 * pw,
...
impl World {
    pub fn artifact_army_mult(&self, p: usize) -> f32 {
        ...
        for a in self.artifacts_of(p) {
            if matches!(self.artifacts[a].kind, ArtifactKind::Blade | ArtifactKind::Banner) {
...
pub fn utter_prophecy(w: &mut World, seer: usize, p: usize) -> Option<usize> {
    ...
    if let Some(cap) = w.polities[p].capital {
        kinds.push((if w.polities[p].at_war() { 1.2 } else { 0.4 }, ProphecyKind::CityBurns(cap, w.cities[cap].times_sacked)));
    }
    kinds.push((0.6, ProphecyKind::RulerMurdered(p)));
    if let Some(s) = w.polities[p].school {
        let n = w.schools[s].influence.values().filter(|v| **v > 0.3).count();
        kinds.push((0.7, ProphecyKind::FaithSpreads(s, n + 2 + rng.below(3))));
    }
    let lost: Vec<usize> = w.artifacts.iter().filter(|a| matches!(a.holder, Holder::Lost) && a.origin == Some(p)).map(|a| a.id).collect();
    ...
    if let Some(&(q, _)) = w.polities[p].neighbors.first() {
        if w.polities[q].alive() {
            kinds.push((0.3 + (1.0 - w.polities[q].stability as f64) * 1.2, ProphecyKind::RealmFalls(q)));
        }
    }
    ...
    let where_ = w.persons[seer].city.map(|c| w.cities[c].name.clone()).unwrap_or_else(|| w.polities[p].short.clone());
    ...
    let text = format!("{} of {} {} that {} before {} winters had passed.", seer_name, where_, opening, what, years);
    let id = w.prophecies.len();
    w.prophecies.push(Prophecy { id, seer, year: w.year, deadline, kind, what: what.clone(), outcome: None, resolved: None });
    let mut refs = vec![Ref::Person(seer), Ref::Polity(p)];
    match kind {
        ProphecyKind::RealmFalls(q) | ProphecyKind::CrownOfEmpire(q) | ProphecyKind::RulerMurdered(q) => {
            if q != p {
...
fn prophecy_what(w: &World, kind: ProphecyKind) -> String {
    match kind {
        ...
        ProphecyKind::CrownOfEmpire(q) => format!("a ruler of {} would wear an emperor's crown", w.polities[q].short),
        ProphecyKind::CityBurns(c, _) => format!("{} would burn", w.cities[c].name),
        ProphecyKind::RulerMurdered(q) => format!("a ruler of {} would die by a hand they trusted", w.polities[q].short),
        ProphecyKind::FaithSpreads(s, n) => format!("{} would be honoured in {} realms", w.schools[s].name, n),
        ProphecyKind::RelicReturns(a, q) => format!("{} would return to {}", w.artifacts[a].name, w.polities[q].short),
    }
}
...
fn prophecy_met(w: &World, pr: &Prophecy) -> bool {
    match pr.kind {
        ...
        ProphecyKind::CityBurns(c, sacked) => w.cities[c].times_sacked > sacked || w.cities[c].destroyed.map(|y| y >= pr.year).unwrap_or(false),
        ProphecyKind::RulerMurdered(q) => w.polities[q].rulers.iter().any(|&r| w.persons[r].died.map(|y| y >= pr.year).unwrap_or(false) && (w.persons[r].death.contains("murdered") || w.persons[r].death.contains("deposed"))),
        ProphecyKind::FaithSpreads(s, n) => w.schools[s].influence.iter().filter(|(p, v)| **v > 0.3 && w.polities[**p].alive()).count() >= n,
...
fn prophecy_refs(w: &World, i: usize) -> Vec<Ref> {
    ...
    match pr.kind {
        ProphecyKind::RealmFalls(q) | ProphecyKind::CrownOfEmpire(q) | ProphecyKind::RulerMurdered(q) => refs.push(Ref::Polity(q)),
        ProphecyKind::CityBurns(c, _) => refs.push(Ref::City(c)),
        ProphecyKind::FaithSpreads(s, _) => refs.push(Ref::School(s)),
...
pub fn tick_legends(w: &mut World) {
    ...
    for p in w.living_polities() {
        ...
        if per.traits.cruelty > 0.8 && reign >= 8 && per.epithet.is_none() && rng.chance(0.06) {
            w.persons[r].epithet = Some(rng.pick(&["the Tyrant", "the Cruel", "the Butcher", "Bloodhand", "the Merciless"]).to_string());
            ...
            w.log(1, EventKind::Politics, &[Ref::Person(r), Ref::Polity(p)], w.capital_cell(p), text);
        }
    }
    // Legends: generals and notables of great renown are remembered at death.
    for i in 0..w.persons.len() {
        let per = &w.persons[i];
        if per.died != Some(w.year) || per.role == Role::Ruler || per.renown < 5.0 || per.epithet.is_some() {
            continue;
        }
        let role = per.role;
        let epithet = match role {
            Role::General => *rng.pick(&["the Unbroken", "Shieldbreaker", "the Hammer", "Stormcrow", "of the Hundred Battles"]),
...
impl World {
    pub fn children_of(&self, r: usize) -> Vec<usize> {
        self.persons.iter().filter(|p| p.parent == Some(r)).map(|p| p.id).collect()
    }
