...
pub const LIST_TABS: [&str; 9] = ["Realms", "Cities", "Peoples", "Schools", "Persons", "Wars", "Places", "Relics", "Prophecies"];
...
fn labelled(out: &mut Vec<Line>, label: &str, text: &str, width: usize, fg: Rgb) {
    ...
    for (k, l) in body.into_iter().enumerate() {
        let pre = if k == 0 { format!("{:<lw$}", label, lw = lw) } else { " ".repeat(lw) };
...
pub fn ref_color(w: &World, r: Ref) -> Rgb {
    match r {
        ...
        Ref::City(c) => w.cities[c].polity.map(|p| w.polities[p].color).unwrap_or(DIMC),
        Ref::Culture(c) => w.cultures[c].color,
        Ref::School(s) => w.schools[s].color,
        Ref::Person(p) => w.persons[p].polity.map(|p| w.polities[p].color).unwrap_or(FG),
...
pub fn entity_loc(w: &World, r: Ref) -> Option<usize> {
    match r {
        ...
        Ref::Person(p) => {
            let per = &w.persons[p];
            per.city.map(|c| w.cities[c].cell).or_else(|| per.polity.and_then(|pp| w.capital_cell(pp)))
        }
        Ref::War(x) => w.capital_cell(w.wars[x].defender).or_else(|| w.capital_cell(w.wars[x].attacker)),
...
pub fn summary(w: &World, r: Ref) -> Vec<(String, Rgb)> {
    ...
    match r {
        Ref::Polity(p) => {
            ...
            out.push((format!("{} · {} lands · {:.0}k people", pol.kind.name(), pol.cells, pol.pop), FG));
            out.push((format!("stability {:.0}%  army {:.0}", pol.stability * 100.0, pol.army), FG));
            if let Some(s) = pol.school {
                out.push((w.schools[s].name.clone(), w.schools[s].color));
            }
            for &wid in &pol.wars {
                let war = &w.wars[wid];
                let other = if war.attacker == p { war.defender } else { war.attacker };
                out.push((format!("at war with {}", w.polities[other].short), Rgb(240, 110, 100)));
            }
        }
        Ref::City(c) => {
            let city = &w.cities[c];
            out.push((city.name.clone(), Rgb(255, 255, 255)));
            out.push((format!("{:.1}k people, founded {}", city.pop, city.founded), FG));
        ...
        Ref::Culture(c) => {
            ...
            out.push((format!("a {} people · {} lands", w.races[cu.race].adj, cu.cells), FG));
        ...
        Ref::School(s) => {
            ...
            out.push((format!("followed in {} realms", sc.influence.values().filter(|v| **v > 0.1).count()), DIMC));
        }
        Ref::Person(p) => {
            let per = &w.persons[p];
        ...
        Ref::War(x) => {
            ...
            out.push((format!("{} vs {}", w.polities[war.attacker].short, w.polities[war.defender].short), FG));
        }
        Ref::Race(x) => {
        ...
        Ref::Feature(f) => {
            ...
            out.push((format!("{} · {} cells", ft.kind.label(), ft.cells.len()), DIMC));
        }
        Ref::Artifact(a) => {
...
pub fn list_rows(w: &World, tab: usize) -> Vec<(String, Ref)> {
    ...
    match tab {
        0 => {
            ...
            ps.sort_by_key(|&p| (w.polities[p].fell.is_some(), std::cmp::Reverse(w.polities[p].cells), std::cmp::Reverse(w.polities[p].peak_cells)));
            for p in ps {
                let pol = &w.polities[p];
                ...
                let row = format!("{:<40}{:<12}{:>6}  {:>7.0}k  {:>3.0}%   {}", clip(&pol.name, 39), pol.kind.name(), status, pol.pop, pol.stability * 100.0, if pol.alive() { w.ruler_short(p) } else { String::new() });
                rows.push((row, Ref::Polity(p)));
            }
        }
        1 => {
            let mut cs: Vec<usize> = (0..w.cities.len()).collect();
            cs.sort_by(|&a, &b| (w.cities[a].destroyed.is_some(), -w.cities[a].pop).partial_cmp(&(w.cities[b].destroyed.is_some(), -w.cities[b].pop)).unwrap());
            for c in cs {
                let city = &w.cities[c];
                let realm = match city.destroyed {
                    Some(y) => format!("destroyed {}", y),
                    None => city.polity.map(|p| w.polities[p].name.clone()).unwrap_or_else(|| "independent".into()),
                };
                let row = format!("{:<24}{:<32}{:>7.1}k  {:>6}", clip(&city.name, 23), clip(&realm, 31), city.pop, city.founded);
                rows.push((row, Ref::City(c)));
            }
        }
        2 => {
            let mut cs: Vec<usize> = (0..w.cultures.len()).collect();
            cs.sort_by_key(|&c| (w.cultures[c].extinct.is_some(), std::cmp::Reverse(w.cultures[c].cells)));
            for c in cs {
                let cu = &w.cultures[c];
                ...
                let row = format!("{:<24}{:<14}{:>6}  {:>7.0}k  {}", clip(&cu.plural, 23), clip(&w.races[cu.race].name, 13), status, cu.pop, cu.lang.name);
                rows.push((row, Ref::Culture(c)));
            }
        }
        3 => {
            let mut ss: Vec<usize> = (0..w.schools.len()).collect();
            ss.sort_by(|&a, &b| (w.schools[a].extinct.is_some(), -w.schools[a].total_influence()).partial_cmp(&(w.schools[b].extinct.is_some(), -w.schools[b].total_influence())).unwrap());
            for s in ss {
                let sc = &w.schools[s];
                ...
                let row = format!("{:<36}{:<12}{:>6}  {}", clip(&sc.name, 35), sc.kind.name(), status, w.cities[sc.home_city].name);
        ...
        4 => {
            ...
            ps.sort_by(|&a, &b| {
                let ka = (w.persons[a].died.is_some(), -w.persons[a].renown, -(w.persons[a].born as f32));
                let kb = (w.persons[b].died.is_some(), -w.persons[b].renown, -(w.persons[b].born as f32));
                ka.partial_cmp(&kb).unwrap()
            });
            for p in ps.into_iter().take(400) {
                let per = &w.persons[p];
                let realm = per.polity.map(|pp| w.polities[pp].short.clone()).unwrap_or_default();
                let died = per.died.map(|y| y.to_string()).unwrap_or_else(|| "alive".into());
                let row = format!("{:<30}{:<13}{:<22}{:>6}  {:>5}", clip(&per.full_name(), 29), per.role.name(), clip(&realm, 21), per.born, died);
                rows.push((row, Ref::Person(p)));
            }
        }
        5 => {
            let mut ws: Vec<usize> = (0..w.wars.len()).collect();
            ws.sort_by_key(|&x| (w.wars[x].ended.is_some(), std::cmp::Reverse(w.wars[x].started)));
            for x in ws {
                let war = &w.wars[x];
                ...
                let row = format!("{:<42}{:<32}{}", clip(&war.name, 41), clip(&format!("{} vs {}", w.polities[war.attacker].short, w.polities[war.defender].short), 31), years);
                rows.push((row, Ref::War(x)));
            }
        }
        7 => {
            let mut arts: Vec<usize> = (0..w.artifacts.len()).collect();
            arts.sort_by_key(|&a| (matches!(w.artifacts[a].holder, crate::sim::Holder::Lost), w.artifacts[a].made));
            for a in arts {
                let ar = &w.artifacts[a];
                let row = format!("{:<40}{:<10}{:>5}   {}", clip(&ar.name, 39), ar.kind.word().to_lowercase(), ar.made, clip(&w.artifact_holder_name(a), 40));
                rows.push((row, Ref::Artifact(a)));
            }
        }
        8 => {
            let mut prs: Vec<usize> = (0..w.prophecies.len()).collect();
            prs.sort_by_key(|&i| (w.prophecies[i].outcome.is_some(), std::cmp::Reverse(w.prophecies[i].year)));
            for i in prs {
                ...
                let row = format!("{:<66}{:<21}{:>6}  {}", clip(&pr.what, 65), clip(&w.persons[pr.seer].full_name(), 20), pr.deadline, outcome);
                rows.push((row, Ref::Person(pr.seer)));
            }
        }
        _ => {
            let mut fs: Vec<usize> = (0..w.terrain.features.len()).collect();
            fs.sort_by_key(|&f| (w.terrain.features[f].name.is_none(), std::cmp::Reverse(w.terrain.features[f].cells.len())));
            for f in fs {
                let ft = &w.terrain.features[f];
...
fn history(w: &World, r: Ref, width: usize, out: &mut Vec<Line>, limit: usize) {
    ...
    out.push(line("", FG, 0));
    out.push(line(format!("History ({} entries)", ids.len()), ACCENT, BOLD));
    let start = ids.len().saturating_sub(limit);
    if start > 0 {
        out.push(line(format!("  … {} earlier entries omitted", start), DIMC, DIM));
    }
    for &id in &ids[start..] {
        let e = &w.chronicle.events[id];
        let (c, a) = super::event_style(e.kind, e.importance);
        for (k, l) in term::wrap(&e.text, width.saturating_sub(8)).into_iter().enumerate() {
            let prefix = if k == 0 { format!("{:>5}  ", e.year) } else { "       ".into() };
...
pub fn detail_lines(w: &World, r: Ref, width: usize, ascii: bool) -> Vec<Line> {
    ...
    match r {
        Ref::Polity(p) => {
            ...
            let status = match pol.fell {
                Some(y) => format!("A {} that {}; founded {}, fell {}.", pol.kind.name(), pol.fall_cause.trim_end_matches('.'), pol.founded, y),
                None => format!("A {} {} founded in year {}.", w.cultures[pol.culture].adj, pol.kind.name(), pol.founded),
            };
            ...
            if pol.alive() {
                ...
                if let Some(rl) = pol.ruler {
                    ...
                    out.push(line(format!("              {} · age {} · reigning {} years", per.traits.describe(), w.year - per.born, w.year - pol.reign_start), DIMC, 0));
                }
                if !pol.dynasty.is_empty() {
                    out.push(line(format!("    Dynasty   {}", pol.dynasty), FG, 0));
                }
            }
            out.push(line(format!("[c] People    the {} ({} folk), who speak {}", w.cultures[pol.culture].plural, w.races[w.cultures[pol.culture].race].adj, w.cultures[pol.culture].lang.name), LINK, 0));
            if let Some(cap) = pol.capital {
                out.push(line(format!("[k] Capital   {} ({:.1}k)", w.cities[cap].name, w.cities[cap].pop), LINK, 0));
            }
            if let Some(s) = pol.school {
                out.push(line(format!("[s] Doctrine  {} ({})", w.schools[s].name, w.schools[s].kind.name()), LINK, 0));
            }
            if let Some(pp) = pol.parent {
                out.push(line(format!("[p] Sprang from {}", w.polities[pp].name), LINK, 0));
            }
            if pol.alive() {
                out.push(line("", FG, 0));
                out.push(line(format!("    Lands     {} (peak {} in year {})   Cities {}", pol.cells, pol.peak_cells, pol.peak_year, pol.cities.len()), FG, 0));
                out.push(line(format!("    People    {:.0}k   Army {:.1}   Treasury {:.0}   Development {:.2}", pol.pop, pol.army, pol.treasury, pol.dev), FG, 0));
                out.push(line(format!("    Stability {} {:.0}%", bar(pol.stability, 20, ascii), pol.stability * 100.0), FG, 0));
                ...
                if !pol.wars.is_empty() {
                    ...
                    for &wid in &pol.wars {
                        let war = &w.wars[wid];
                        let other = if war.attacker == p { war.defender } else { war.attacker };
                        let lean = if (war.score > 0.0) == (war.attacker == p) { "winning" } else { "losing" };
                        out.push(line(format!("[w] At war    {} against {} since {} ({}, {} battles)", war.name, w.polities[other].name, war.started, lean, war.battles), Rgb(240, 110, 100), 0));
                    }
                }
                ...
                if !nb.is_empty() {
                    let s: Vec<String> = nb
                        ...
                        .map(|&(q, _)| {
                            ...
                            let mood = if t > 0.5 { "hostile" } else if t > 0.3 { "wary" } else { "calm" };
                ...
                if !pol.cities.is_empty() {
                    let names: Vec<String> = pol.cities.iter().map(|&c| format!("{} {:.0}k", w.cities[c].name, w.cities[c].pop)).collect();
                    labelled(&mut out, "    Cities", &names.join(", "), w2, DIMC);
                }
                let schools: Vec<String> = w.schools.iter().filter(|s| s.alive()).filter_map(|s| s.influence.get(&p).map(|v| (s, *v))).filter(|(_, v)| *v > 0.1).map(|(s, v)| format!("{} {:.0}%", s.short, v * 100.0)).collect();
                if !schools.is_empty() {
                    labelled(&mut out, "    Beliefs", &schools.join(", "), w2, DIMC);
                }
                let relics: Vec<String> = w.artifacts_of(p).iter().map(|&a| w.artifacts[a].name.clone()).collect();
                if !relics.is_empty() {
                    labelled(&mut out, "    Relics", &relics.join(", "), w2, Rgb(255, 200, 80));
                }
                ...
                if !pending.is_empty() {
                    labelled(&mut out, "    Foretold", &pending.join("; "), w2, Rgb(200, 140, 240));
                }
                out.push(line("", FG, 0));
                out.push(line("[x] Intervene with the Hand of Fate", Rgb(200, 140, 240), 0));
            }
            if pol.rulers.len() > 1 {
                out.push(line("", FG, 0));
                ...
                for &rl in pol.rulers.iter().rev().take(12) {
                    let per = &w.persons[rl];
                    let end = per.died.map(|y| y.to_string()).unwrap_or_else(|| "now".into());
                    out.push(line(format!("  {:<34} {}", per.full_name(), if per.died.is_some() { format!("{}: {}", end, per.death) } else { "reigning".into() }), FG, 0));
                }
            }
        ...
        Ref::City(ci) => {
            ...
            let mut desc = format!("A city of the {}, founded in year {}", w.cultures[city.culture].plural, city.founded);
            if let Some(y) = city.destroyed {
                desc.push_str(&format!(", destroyed in year {}", y));
            }
            desc.push('.');
            out.push(line(desc, FG, 0));
            out.push(line(format!("    Terrain   {}", w.terrain.describe_cell(city.cell)), DIMC, 0));
            out.push(line("", FG, 0));
            if let Some(p) = city.polity {
                let cap = if w.polities[p].capital == Some(ci) { " (capital)" } else { "" };
                out.push(line(format!("[p] Realm     {}{}", w.polities[p].name, cap), LINK, 0));
            }
            out.push(line(format!("[c] People    the {}", w.cultures[city.culture].plural), LINK, 0));
            ...
            let schools: Vec<String> = w.schools.iter().filter(|s| s.home_city == ci).map(|s| s.name.clone()).collect();
            if !schools.is_empty() {
                out.push(line(format!("    Birthplace of {}", schools.join(", ")), Rgb(200, 140, 240), 0));
            }
            history(w, r, width, &mut out, 300);
        }
        Ref::Culture(cu) => {
            let c = &w.cultures[cu];
            out.push(line(format!("THE {}", c.plural.to_uppercase()), c.color, BOLD));
            let race = &w.races[c.race];
            let mut desc = format!("A {} people. They speak {} and call themselves {}.", race.adj, c.lang.name, c.name);
            ...
            for l in term::wrap(&format!("The {} are {}.", race.plural, race.description), w2) {
                out.push(line(l, DIMC, 0));
            }
            out.push(line("", FG, 0));
            out.push(line(format!("[a] Race      the {}", race.plural), LINK, 0));
            if let Some(p) = c.parent {
                out.push(line(format!("[p] Descended from the {}", w.cultures[p].plural), LINK, 0));
            }
            let v = c.values;
            ...
            out.push(line(format!("    Lands     {}   People {:.0}k   Since year {}", c.cells, c.pop, c.founded), FG, 0));
            let realms: Vec<String> = w.polities.iter().filter(|p| p.alive() && p.culture == cu).map(|p| p.name.clone()).collect();
            if !realms.is_empty() {
                labelled(&mut out, "    Realms", &realms.join(", "), w2, DIMC);
            }
            let children: Vec<String> = w.cultures.iter().filter(|x| x.parent == Some(cu)).map(|x| x.plural.clone()).collect();
            if !children.is_empty() {
                labelled(&mut out, "    Offshoots", &children.join(", "), w2, DIMC);
            }
            out.push(line("", FG, 0));
            let sample: Vec<String> = (0..6).map(|_| c.lang.place(&w.rng)).collect();
            out.push(line(format!("    Names in {}: {}", c.lang.name, sample.join(", ")), DIMC, 0));
            history(w, r, width, &mut out, 200);
        }
        Ref::School(s) => {
            let sc = &w.schools[s];
            out.push(line(sc.name.to_uppercase(), sc.color, BOLD));
            let mut desc = format!("{} {} founded in year {} in {}; it {}.", article(sc.kind.name()), sc.kind.name(), sc.founded, w.cities[sc.home_city].name, sc.doctrine);
            if let Some(y) = sc.extinct {
                desc.push_str(&format!(" Its last {} died in year {}.", sc.kind.follower(), y));
            }
            for l in term::wrap(&desc, w2) {
                out.push(line(l, FG, 0));
            }
            out.push(line("", FG, 0));
            out.push(line(format!("[f] Founder   {}", w.persons[sc.founder].full_name()), LINK, 0));
            out.push(line(format!("[k] Home      {}", w.cities[sc.home_city].name), LINK, 0));
            if let Some(p) = sc.parent {
                out.push(line(format!("[p] Schism of {}", w.schools[p].name), LINK, 0));
            }
            out.push(line("", FG, 0));
            ...
            out.push(line("", FG, 0));
            let mut infl: Vec<(usize, f32)> = sc.influence.iter().map(|(&p, &v)| (p, v)).filter(|(p, v)| *v > 0.05 && w.polities[*p].alive()).collect();
            infl.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
            out.push(line(format!("Following (hostility {:.0}%)", sc.hostility * 100.0), ACCENT, BOLD));
            for (p, v) in infl.iter().take(15) {
                let state = if w.polities[*p].school == Some(s) { " (state doctrine)" } else { "" };
                out.push(line(format!("  {} {:.0}%  {}{}", bar(*v, 10, ascii), v * 100.0, w.polities[*p].name, state), FG, 0));
            }
            let children: Vec<String> = w.schools.iter().filter(|x| x.parent == Some(s)).map(|x| x.name.clone()).collect();
            if !children.is_empty() {
        ...
        Ref::Person(pi) => {
            ...
            let mut desc = format!("A {} {} of the {}, born in year {}", per.traits.describe(), per.role.name(), w.cultures[per.culture].plural, per.born);
            match per.died {
                Some(y) => desc.push_str(&format!("; {} in year {} at the age of {}.", per.death.trim_end_matches('.'), y, y - per.born)),
            ...
            if let Some(p) = per.polity {
                out.push(line(format!("[p] Realm     {}", w.polities[p].name), LINK, 0));
            }
            out.push(line(format!("[c] People    the {}", w.cultures[per.culture].plural), LINK, 0));
            if let Some(s) = per.school {
                out.push(line(format!("[s] School    {}", w.schools[s].name), LINK, 0));
            }
            ...
            if per.battles_won > 0 {
                out.push(line(format!("    Battles   {} won", per.battles_won), DIMC, 0));
            }
            let held: Vec<String> = w.artifacts.iter().filter(|ar| ar.holder == crate::sim::Holder::Person(pi)).map(|ar| ar.name.clone()).collect();
            if !held.is_empty() {
                labelled(&mut out, "    Bears", &held.join(", "), w2, Rgb(255, 200, 80));
            }
            let prophecies: Vec<String> = w
                .prophecies
                .iter()
                .filter(|pr| pr.seer == pi)
                .map(|pr| format!("{} ({})", pr.what, match pr.outcome { Some(true) => "came true", Some(false) => "failed", None => "pending" }))
                .collect();
            if !prophecies.is_empty() {
                labelled(&mut out, "    Foretold", &prophecies.join("; "), w2, Rgb(200, 140, 240));
            }
            let t = per.traits;
            out.push(line(format!("    ambition {}  valor {}  wisdom {}", bar(t.ambition, 8, ascii), bar(t.valor, 8, ascii), bar(t.wisdom, 8, ascii)), DIMC, 0));
            out.push(line(format!("    piety    {}  cruelty {} charisma {}", bar(t.piety, 8, ascii), bar(t.cruelty, 8, ascii), bar(t.charisma, 8, ascii)), DIMC, 0));
            history(w, r, width, &mut out, 200);
        }
        Ref::War(x) => {
            let war = &w.wars[x];
            ...
            let status = match war.ended {
                Some(e) => format!("Fought {}-{} between {} and {}. It {}", war.started, e, w.polities[war.attacker].name, w.polities[war.defender].name, war.result),
                None => format!("Fought since {} between {} and {}. {} battles so far.", war.started, w.polities[war.attacker].name, w.polities[war.defender].name, war.battles),
            };
            for l in term::wrap(&status, w2) {
                out.push(line(l, FG, 0));
            }
            ...
            out.push(line("", FG, 0));
            out.push(line(format!("[a] Attacker  {}", w.polities[war.attacker].name), LINK, 0));
            out.push(line(format!("[d] Defender  {}", w.polities[war.defender].name), LINK, 0));
            history(w, r, width, &mut out, 300);
        }
        Ref::Race(x) => {
            let race = &w.races[x];
            out.push(line(format!("THE {}", race.plural.to_uppercase()), ACCENT, BOLD));
            for l in term::wrap(&format!("The {} are {}.", race.plural, race.description), w2) {
                out.push(line(l, FG, 0));
            }
            out.push(line(format!("    Typical lifespan {} years.", race.lifespan as i32), DIMC, 0));
            let peoples: Vec<String> = w.cultures.iter().filter(|c| c.race == x && c.extinct.is_none()).map(|c| c.plural.clone()).collect();
        ...
        Ref::Artifact(a) => {
            ...
            let mut desc = format!("A {}, {}. Made in year {}", ar.kind.word().to_lowercase(), ar.description, ar.made);
            ...
            out.push(line("", FG, 0));
            out.push(line(format!("[h] Held by   {}", w.artifact_holder_name(a)), LINK, 0));
            if let Some(m) = ar.maker {
                out.push(line(format!("[k] Maker     {}", w.persons[m].full_name()), LINK, 0));
            }
            if let Some(o) = ar.origin {
                out.push(line(format!("[o] Origin    {}", w.polities[o].name), LINK, 0));
            }
            out.push(line(format!("    Power     {}", bar(ar.power, 10, ascii)), DIMC, 0));
            history(w, r, width, &mut out, 200);
        }
        Ref::Feature(f) => {
            let ft = &w.terrain.features[f];
...
pub fn hand_of_fate(w: &mut World, p: usize, choice: u8) -> String {
    ...
    match choice {
        ...
        1 => {
            ...
            w.plagues.push(crate::sim::Plague { name: "the Whispering Sickness".into(), years_left: 3, polities: vec![p], deaths: 0.0 });
            let text = format!("Crows gathered on the roofs of {} for a month. Then the coughing began.", w.polities[p].capital.map(|c| w.cities[c].name.clone()).unwrap_or_else(|| w.polities[p].short.clone()));
            w.log(2, EventKind::Disaster, &[Ref::Polity(p)], cap, text);
            format!("cursed {}", name)
        }
        2 => {
            if let Some(c) = w.polities[p].capital {
                ...
                let kind = if mana > 0.45 { SchoolKind::Arcane } else if w.rng.chance(0.5) { SchoolKind::Divine } else { SchoolKind::Philosophical };
                ...
                w.log(2, EventKind::Magic, &[Ref::School(s), Ref::Person(founder), Ref::Polity(p), Ref::City(c)], cap, text);
        ...
        _ => {
            ...
            crate::sim::politics::ruler_dies(w, p, "died quietly in the night, as if called away.".into(), 1);
            format!("{} has died", who)
        }
    }
}
